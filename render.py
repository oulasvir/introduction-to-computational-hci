    ...
    def colorize(self,associations):
        ...
        for i in range(0, len(self.labels)):
            for j in range(i+1, len(self.labels)):
                if associations.get(self.labels[i]+self.labels[j]): association = associations.get(self.labels[i]+self.labels[j])
                elif associations.get(self.labels[j]+self.labels[i]): association = associations.get(self.labels[j]+self.labels[i])
                else: association = 0
